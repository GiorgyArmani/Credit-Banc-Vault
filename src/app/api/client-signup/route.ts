...
export async function POST(request: Request) {
  try {
    ...
    // Tags condicionales basados en situaciones especiales
    if (body.has_defaulted_mca) {
      tags_to_apply.push('has_mca_default');
    }
    
    if (body.owns_real_estate) {
      tags_to_apply.push('owns_real_estate');
    }
    
    if (body.has_tax_liens) {
      tags_to_apply.push('has_tax_liens');
    }
    
    if (body.has_bankruptcy_foreclosure_3y) {
      tags_to_apply.push('has_bankruptcy_foreclosure');
    }
    
    // Aplicar tags en GHL
    await ghl_add_tags(ghl_contact_id, tags_to_apply);
    console.log(` Tags aplicados en GHL: ${tags_to_apply.join(', ')}`);

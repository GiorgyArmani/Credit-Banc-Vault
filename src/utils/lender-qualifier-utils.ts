...
function evaluateFicoRequirement(
  ...
  if (lender_criteria.min_fico === null) {
    results.passed.push('No FICO requirement');
    return true;
  }
...
function evaluateFinancialHistory(
  ...
  if (lender_criteria.allows_bankruptcies === false && 
      client_profile.has_bankruptcy_foreclosure_3y) {
    results.failed.push('Recent bankruptcy/foreclosure not allowed');
    passes = false;
  } else if (client_profile.has_bankruptcy_foreclosure_3y) {
    results.warnings.push('Client has recent bankruptcy/foreclosure');
  } else {
...
export function evaluateLenderQualification(
  ...
  const context: EvaluationContext = {
    ...
    numeric_credit_score: creditScoreToNumeric(
